...
/**
 * This script adds predefined itineraries for a batch of destinations without existing itineraries
 */
async function addBatchItineraries() {
  try {
    console.log("Starting to add a batch of destination itineraries with preset data...");
    
    // Find the next 5 destinations without itineraries
    ...
    if (destinationsWithoutItineraries.length === 0) {
      console.log("No destinations without itineraries found!");
      return;
    }
    
    console.log(`Processing ${destinationsWithoutItineraries.length} destinations...`);
    ...
    console.error("Error adding batch destination itineraries:", error);
  }
}

// Run the function
addBatchItineraries().then(() => {
  console.log('Script completed');
  process.exit(0);
}).catch(error => {
  console.error("Failed to add batch destination itineraries:", error);
  process.exit(1);
});